...
/**
 * Created by dingsigang on 16-7-26.
 */
public class NDKTestUtil {
    ...
    public static void compressBitmap(Bitmap image, String filePath) {
        ...
        int maxSize = 100;
        // 获取尺寸压缩倍数
        int ratio = NDKTestUtil.getRatioSize(image.getWidth(), image.getHeight());
        // 压缩Bitmap到对应尺寸
        ...
        // JNI保存图片到SD卡 这个关键
        NDKTestUtil.saveBitmap(result, options, filePath, true);
        // 释放Bitmap
    ...
    private static void saveBitmap(Bitmap bit, int quality, String fileName, boolean optimize) {
        test(bit, bit.getWidth(), bit.getHeight(), quality, fileName.getBytes(), optimize);
    }
     ...
     * @version V1.0.0
     */
    private static native String test(Bitmap bit, int w, int h, int quality, byte[] fileNameBytes,
                                                boolean optimize);
    /**
     * 加载lib下两个so文件
     */
